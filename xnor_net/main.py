...
from png2dataset import ImageDataset, MaskDataset
...
def proj_test(evaluate=False):
    ...
    bin_op.binarization()
    with torch.no_grad():
        for data, target in proj_loader:
            if args.cuda:
                data, target = data.cuda(), target.cuda()
            data, target = Variable(data), Variable(target)
            output = model(data)
    ...
    acc = 100. * float(correct) / len(proj_loader.dataset)
    if (acc > best_acc):
        best_acc = acc
        if not evaluate:
            save_state(model, best_acc)

    test_loss /= len(proj_loader.dataset)
    print('\nTest set: Average loss: {:.4f}, Accuracy: {}/{} ({:.2f}%)'.format(
        test_loss * args.batch_size, correct, len(proj_loader.dataset),
        100. * float(correct) / len(proj_loader.dataset)))
    return
...
if __name__ == '__main__':
    ...
    parser.add_argument('--weight-decay', '--wd', default=1e-5,
                        type=float, metavar='W', help='weight decay (default: 1e-5)')
    ...
    parser.add_argument('--pretrained', action='store',
                        default=None, help='pretrained model')
    parser.add_argument('--evaluate', action='store_true',
                        default=False, help='whether to run evaluation')
    parser.add_argument('--fastMCD', action='store',
                        default='data/number_results', help='whether to run on fastMCD data')
    parser.add_argument('--mask', action='store_true',
                        default=False, help='whether to use image or masks on fastMCD')
    ...
    if args.mask:
        proj_loader = torch.utils.data.DataLoader(
            MaskDataset(args.fastMCD, transform=tform),
            batch_size=args.test_batch_size, shuffle=True, **kwargs)
    else:
        proj_loader = torch.utils.data.DataLoader(
            ImageDataset(args.fastMCD, thresh=160, transform=tform),
            batch_size=args.test_batch_size, shuffle=True, **kwargs)
    ...
    if not args.pretrained:
        ...
        print(model.state_dict()['bin_ip1.bn.weight'].shape)
        print(pretrained_model['state_dict']['bin_ip1.bn.weight'].shape)
    ...
    for key, value in param_dict.items():
        params += [{'params': [value], 'lr': args.lr,
                    'weight_decay': args.weight_decay, 'key': key}]
